...
function PasswordField({
  ...
  placeholder: string;
  register: ReturnType<
    typeof useForm<SignInFormValues | SignUpFormValues>
  >["register"];
  showPassword: boolean;
        ...
        <div className="flex items-start space-x-3">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              Account creation is enabled with email and password only. Google
      ...
      <div className="relative overflow-hidden rounded-2xl border border-border/60 bg-card/90 shadow-xl backdrop-blur-sm">
        ...
        <div className="relative z-10">
          ...
          <AnimatePresence mode="wait">
            {defaultView === authViews.SIGN_IN ? (
              <AuthSignIn
                onSignUp={() => handleViewChange(authViews.SIGN_UP)}
              />
            ) : (
              <AuthSignUp
                onSignIn={() => handleViewChange(authViews.SIGN_IN)}
              />
            )}
          </AnimatePresence>
        </div>
